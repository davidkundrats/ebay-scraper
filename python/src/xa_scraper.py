        ...
        with requests.Session() as session:  # open session
            ...
            for date in dateTags:  # need to filter out the second span tag identified by class 'clipped' use span tag identified by 'POSITIVE' instead
                ...
                for i in range(0, len(extracted)):
                    ...
                    itemDate.append(extracted[i].get_text())

            # need to get rid of bogus first element in both lists
            if (len(nameTags) == len(priceTags)):
                nameTags.pop(0)
                priceTags.pop(0)
            ...
            if (len(nameTags) == len(priceTags) == len(itemDate)):
                ...
                raise Exception(
                    'item count + price count and date count mismatch - unable to tabulate data')
...
def displayComplete():
    ...
    label = ui.CTkLabel(
        master=popup, text="Generated Dataframe. Process complete! Plot options below or save as CSV")
    label.pack(pady=12, padx=9)
    pathEntry = ui.CTkEntry(
        master=popup, placeholder_text="Set path to save csv")
    pathEntry.pack(pady=20, padx=16)
    plotButton = ui.CTkButton(
        master=popup, text="Average Price", command=avgPrice)
    plotButton.pack(pady=24, padx=18)
    saveToCSVButton = ui.CTkButton(
        master=popup, text="Save", command=saveAsCSV)
